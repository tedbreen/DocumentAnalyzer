...
function stringAnalyzer(str) {
    ...
    paragraphs.forEach(function(paragraph) {
        ...
        var sentences = sentenceSplit(cleanParagraph)._select(function(sentence) {
        ...
        sentences.forEach(function(sentence) {
            counts.words += sentence.split(" ").length;
        });
    })
    return counts;
}
...
function sentenceSplit(str) {
    ...
    var sentences = [];
    var sentence = "";
    for (var idx = 0; idx < cleanStr.length; idx++) {
        if (sentenceCompleters[cleanStr[idx]]) {
            if (!(numberStrings[cleanStr[idx - 1]] && numberStrings[cleanStr[idx + 1]])) {
                sentences.push(sentence);
                sentence = "";
            }
        } else {
            if (!((cleanStr[idx] === " ") && (sentence.length === 0))) {
                sentence += cleanStr[idx]
            }
        }
    }
    return sentences;
...
function appendCounts(elArray, countObj) {
    var el, countDisplayEl, countDisplayNode;
    for (el = 0; el < elArray.length; el++) {
        countDisplayEl = elArray[el];
        countDisplayNode = document.createTextNode(countObj[elArray[el].classList[1]]);
